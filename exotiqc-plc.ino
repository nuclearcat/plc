...
T-Sensor: NOSHOK 800-0/250-1-1-8-8-040-6 (RTD: PT100)
*/

/* User defined variables */
double desiredTemperature = 20.0; // in Celsius!!!! I know, I know, but I'm not a monster to use Fahrenheit
long PIDPeriod = 30000; // in milliseconds
...
// RTD lookup taken from https://github.com/drhaney/pt100rtd
...
#define PT100_TABLE_SIZE sizeof(PT100_table)/sizeof(PT100_table[0])
#define PT_MIN_RESISTANCE 1852
#define PT_MAX_RESISTANCE 39048

#define DEBUG 1

enum {
    FAULT_NONE = 0,
    FAULT_RTD_SENSOR_SHORT = 1,
    FAULT_RTD_SENSOR_OPEN = 2,
};
...
void onHourChange() {
    currentStart = (currentStart + 1) % NUM_ELEMENTS;
...
/* Get temperature from sensor, with precision of 1C
   this we dont mess with polynomial interpolation
*/
double getTemperature(int resistance) {
    if (resistance < PT_MIN_RESISTANCE) {
        FaultFlag |= FAULT_RTD_SENSOR_SHORT;
        return -200;
    } else if (resistance > PT_MAX_RESISTANCE) {
        FaultFlag |= FAULT_RTD_SENSOR_OPEN;
        return 850;
    }
    for (int i = 0; i < PT100_TABLE_SIZE; i++) {
        if (resistance < pgm_read_word_near(PT100_table + i)) {
            return i - 200;
        }
    }
    return 0;
...
class PID {
    private:
        double kp, ki, kd;
        double setpoint, integral, prevError;
        double prevTime;
...
// TODO: compute PID parameters, make separate routine for autotuning?
PID pid(1, 0.1, 0.1);
long lastPIDTime = 0;
...
void setup() {
    ...
    // TODO: read from EEPROM last value in cast of power failure
    setPower(0);
    pid.setSetpoint(desiredTemperature);
}

// TODO: implement calibration routine
// We set initial temperature to 20C, turn on 1 element, wait for 5 minutes, measure temperature
// This we get proportional gain (kp) using formula kp = (T2 - T1) / (P2 - P1)
// Validate by turning on 2nd element, wait for 5 minutes, measure temperature
// calculate kp2 = (T3 - T2) / (P3 - P2)
void calibrate() {
...
void loop() {
    ...
    if (millis() - lastPIDTime > PIDPeriod) {
        int power = pid.compute(temperature);
        setPower(power);
        lastPIDTime = millis();
    }
    // TODO: remove?
